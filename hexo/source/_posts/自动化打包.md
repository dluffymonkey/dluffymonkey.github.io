...
Options:
  ...
  -u isuploadpgyer, --u=isuploadpgyer
                        0 or 1, Whether or not upload to Pgyer
```

<!-- more -->

第一种使用方式：在项目文件目录下打开命令执行下面的命令
...
第二种使用方式：在py文件的目录下打开命令执行下面的命令
```python
python autobuild.py -p ../yourscheme.xcodeproj -s yourscheme
python autobuild.py -w ../yourworkspace.xcworkspace -s yourscheme
```

上面的参数是必选的，还有一些不是必选的，如`-a`只有当你的项目名称和导出来的包名不一致是使用的，表示`archivename`，这个设置是在Xcode里面的`Edit Scheme`下面的最后一个选项`archive`中的`Archive Name`参数决定的；`-u`表示是否上传蒲公英，0表示不上传，1表示上传，不传则不上传；`-m`表示上传蒲公英的时候的描述。
